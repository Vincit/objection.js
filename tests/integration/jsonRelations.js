      ...
      describe('belongs to one relation', () => {
        it('insert', () => {
          return Person.query()
            ...
            .then(person => {
              expect(person.json.stuff.favoritePetId).to.equal(
                person.favoritePet.id
              );
            });
        });
        ...
        it('patch', () => {
          return Person.query()
            .findOne({name: 'Arnold'})
            .then(it =>
              it.$relatedQuery('favoritePet').patch({json: {updated: true}})
            )
            .then(() =>
              Animal.query()
                .select('json', 'name')
        ...
        it('relate', () => {
          return Person.query()
            ...
            .then(people => {
              ...
              expect(brad.json.stuff.favoritePetId).to.equal(
                brad.favoritePet.id
              );
            });
        });
      ...
      describe('has many relation', () => {
        it('insert', () => {
          return Animal.query()
            .findOne({name: 'Fluffy'})
            .then(it =>
              it.$relatedQuery('peopleWhoseFavoriteIAm').insert({name: 'Jorge'})
            )
            .then(() =>
              Animal.query()
                .findOne({name: 'Fluffy'})
        ...
        it('find', () => {
          return Animal.query()
            .findOne({name: 'Fluffy'})
            .then(it =>
              it.$relatedQuery('peopleWhoseFavoriteIAm').select('name')
            )
            .then(pet => {
              expect(pet).to.eql([
                {
        ...
        it('relate', () => {
          return Animal.query()
            ...
            .spread((fluffy, brad) =>
              fluffy.$relatedQuery('peopleWhoseFavoriteIAm').relate(brad)
            )
            .then(() =>
              Animal.query()
                .findOne({name: 'Fluffy'})
      ...
      describe('many to many relation', () => {
        it('insert', () => {
          return Person.query()
            .findOne({name: 'Brad'})
            .then(it =>
              it.$relatedQuery('movies').insert({name: 'Seven years in Tibet'})
            )
            .then(() =>
              Person.query()
                .findOne({name: 'Brad'})
        ...
        it('patch', () => {
          return Person.query()
            .findOne({name: 'Arnold'})
            .then(it =>
              it.$relatedQuery('movies').patch({name: 'Some terminator'})
            )
            .then(() =>
              Movie.query()
                .select('name')
        ...
        it('relate', () => {
          return Person.query()
            ...
            .then(it => [
              it,
              Movie.query().findOne({name: 'Inglorious bastards'})
            ])
            .spread((arnold, bastards) =>
              arnold.$relatedQuery('movies').relate(bastards.id)
            )
            .then(() =>
              Person.query()
                .select('name')
