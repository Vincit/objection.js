...
module.exports = session => {
  ...
  describe('Model eager queries', () => {
    ...
    test('model1Relation2.model2Relation2.model1Relation1', models => {
      ...
      expect(models[0]).to.be.a(Model1);
      expect(
        models[0].model1Relation2[0].model2Relation2.model1Relation1
      ).to.be.a(Model1);
    });
    ...
    it('joinEager shorthand', () => {
      return Model1.query()
        ...
        .modify(builder => {
          expect(builder.eagerOperationFactory()).to.equal(
            Model1.JoinEagerAlgorithm
          );
        })
        .then(model => {
          expect(model).to.eql({
    ...
    it('keepImplicitJoinProps', () => {
      return Model1.query()
        ...
        .modifyEager('model1Relation2', qb =>
          qb.select('model_2.id_col').orderBy('model_2.id_col')
        )
    ...
    describe('JoinEagerAlgorithm', () => {
      ...
      it('relation references longer that 63 chars should throw an exception', done => {
        Model1.query()
          .where('Model1.id', 1)
          .eager(
            '[model1Relation1.model1Relation1.model1Relation1.model1Relation1]'
          )
          .eagerAlgorithm(Model1.JoinEagerAlgorithm)
          .then(() => {
        ...
        return Promise.all(
          [
            ...
            return Model1.query()
              ...
              .eager(
                'model1Relation2.model2Relation1.[model1Relation1, model1Relation2]'
              )
              ...
              .then(models => {
                ...
                expect(models[0].model1Relation2[0].model2Prop1).to.equal(
                  'hejsan 2'
                );

                expect(
                  models[0].model1Relation2[0].model2Relation1
                ).to.have.length(1);
                expect(
                  models[0].model1Relation2[0].model2Relation1[0].id
                ).to.equal(6);
              });
          })
        );
        ...
        return Promise.all(
          [
            ...
            return Model1.query()
              ...
              .filterEager(
                'model1Relation2.model2Relation1.model1Relation1',
                builder => {
                  builder.select('model1Prop1');
                }
              )
              .filterEager(
                'model1Relation2.model2Relation1.model1Relation2',
                builder => {
                  builder.select('model_2_prop_1');
                }
              )
              .then(models => {
                models[0].model1Relation2 = _.sortBy(
                  models[0].model1Relation2,
                  'model2Prop1'
                );
    ...
    describe('Multiple parents + ManyToManyRelation', () => {
      ...
      it('should work with WhereInEagerAlgorithm', () => {
        return Model2.query()
          ...
          .map(model => {
            model.model2Relation1 = _.sortBy(
              model.model2Relation1,
              'model1Prop1'
            );
            return model;
          })
    ...
    if (isPostgres(session.knex)) {
      ...
      it.skip('check JoinEagerAlgorithm generated SQL', () => {
        ...
        let mockKnex = mockKnexFactory(session.knex, function(
          mock,
          then,
          args
        ) {
    ...
    return models => {
      Model1.traverse(models, model => {
        if (model.model1Relation2) {
          model.model1Relation2 = _.sortBy(model.model1Relation2, [
            'idCol',
            'model2Prop1'
          ]);
        }
        ...
        if (model.model2Relation1) {
          model.model2Relation1 = _.sortBy(model.model2Relation1, [
            'id',
            'model1Prop1'
          ]);
        }
