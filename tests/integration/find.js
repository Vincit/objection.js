      ...
      describe('query builder methods', () => {
        it('.select()', () => {
          return Model2.query()
            ...
            .then(models => {
              ...
              expect(
                _.uniq(_.flattenDeep(_.map(models, _.keys))).sort()
              ).to.eql(['$afterGetCalled', 'idCol', 'model2Prop2']);
              expect(_.map(models, 'idCol').sort()).to.eql([1, 2, 3]);
              expect(_.map(models, 'model2Prop2').sort()).to.eql([10, 20, 30]);
        ...
        it('.join()', () => {
          return Model2.query()
            ...
            .then(models => {
              ...
              expect(_.map(models, 'model1Prop1')).to.eql([
                'hello 1',
                'hello 1'
              ]);
            });
        ...
        if (session.isPostgres()) {
          it('smoke test for various methods', () => {
            ...
            return Model2.query()
              ...
              .leftOuterJoin('model_2 as m2', join =>
                join
        ...
        it('raw in select', () => {
          return Model2.query()
            .select(
              'model_2.*',
              raw('?? + ? as ??', 'model_2_prop_2', 10, 'model_2_prop_2')
            )
            .orderBy('id_col')
            .then(models => {
    ...
    describe('joinRelation()', () => {
      ...
      it('should join an eager expression `a.a.b`', () => {
        return Model1.query()
          ...
          .where(
            'model1Relation1:model1Relation1:model1Relation2.model_2_prop_1',
            'hejsan 4'
          )
          .first()
          .then(model => {
      ...
      it('should join an eager expression `[a, b]`', () => {
        return Model1.query()
          .select(
            'Model1.id',
            'Model1.model1Prop1',
            'model1Relation2.model_2_prop_1 as model2Prop1'
          )
          .leftJoinRelation('[model1Relation1, model1Relation2]')
        ...
        describe('knex methods', () => {
          it('.select()', () => {
            return parent1
              ...
              .then(related => {
                expect(related).to.be.a(Model1);
                expect(_.keys(related).sort()).to.eql([
                  '$afterGetCalled',
                  'id'
                ]);
              });
          ...
          return Promise.all([
            parent1
              ...
              .then(related => {
                ...
                expect(_.map(related, 'model2Prop1').sort()).to.eql([
                  'text 1',
                  'text 2',
                  'text 3'
                ]);
        ...
        it('should join the related rows', () => {
          return Model1.getRelation('model1Relation2')
            ...
            .then(models => {
              ...
              expect(_.map(models, 'model_2_prop_1').sort()).to.eql([
                'text 1',
                'text 2',
                'text 3',
                'text 4',
        ...
        describe('knex methods', () => {
          it('.select()', () => {
            return parent1
              ...
              .then(related => {
                ...
                expect(
                  _.uniq(_.flattenDeep(_.map(related, _.keys))).sort()
                ).to.eql(['$afterGetCalled', 'idCol']);
              });
          });
          ...
          it('.orWhere()', () => {
            return parent2
              .$relatedQuery('model1Relation2')
              .where(function() {
                this.where('model_2_prop_2', '=', '1').orWhere(
                  'model_2_prop_2',
                  '=',
                  '3'
                );
          ...
          return Promise.all([
            parent1
              ...
              .then(related => {
                ...
                expect(_.map(related, 'model1Prop1').sort()).to.eql([
                  'blaa 1',
                  'blaa 2',
                  'blaa 3'
                ]);
                expect(_.map(related, 'aliasedExtra').sort()).to.eql([
                  null,
                  null,
                  null
                ]);
