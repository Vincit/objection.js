        ...
        return (
          Model1.query(trx)
            ...
            .then(result => {
              ...
              expect(result.model1Relation2[1].$beforeUpdateCalled).to.equal(
                undefined
              );
              expect(result.model1Relation2[1].$afterUpdateCalled).to.equal(
                undefined
              );
            ...
            .then(result => {
              ...
              return Promise.all([
                ...
                expect(model2Rows.find(it => it.id_col == 2)).to.equal(
                  undefined
                );
              });
            })
      ...
      return transaction(session.knex, trx => {
        return Model1.query(trx)
          ...
          .then(result => {
            ...
            expect(result.model1Relation1.$beforeUpdateCalled).to.equal(
              undefined
            );
            expect(result.model1Relation1.$afterUpdateCalled).to.equal(
              undefined
            );
          });
      })
      ...
      transaction(session.knex, trx => {
        ...
        .catch(err => {
          expect(err.message).to.equal(
            'one or more of the root models (ids=[1000]) were not found'
          );
          return session
            .knex('Model1')
            .whereIn('model1Prop1', [
              'updated root 2',
              'inserted belongsToOne'
            ]);
        })
        .then(rows => {
      ...
      return Model1.query(session.knex)
        ...
        .then(result => {
          ...
          errors.forEach(error => {
            ...
            expect(error.data.allowedRelations).to.equal(
    ...
    describe('validation and transactions', () => {
      ...
      it('should validate (also tests transactions)', () => {
        ...
        return Promise.map(fails, fail => {
          return transaction(session.knex, trx =>
            Model1.query(trx).upsertGraph(fail)
          ).reflect();
        })
          .then(results => {
            ...
            results.forEach(res => {
              ...
              expect(res.reason().data.model1Prop1[0].message).to.equal(
              ...
              return Model1.query(trx)
                ...
                .then(result => {
                  ...
                  return Promise.all([
                    ...
                    // Row 5 should be deleted.
                    expect(model1Rows.find(it => it.id == 5)).to.equal(
                      undefined
                    );
                    // Row 6 should NOT be deleted even thought its parent is.
                    expect(model1Rows.find(it => it.id == 6)).to.be.an(Object);
