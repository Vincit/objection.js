  ...
  describe('JSON queries', () => {
      ...
      describe('Querying rows', () => {
        ...
        it('should be able to use ref with join', () => {
          ...
          return BoundModel.query()
            .join(
              'ModelJson as t2',
              ref('ModelJson.jsonArray:[0]'),
              '=',
              ref('t2.jsonObject:attr')
            )
            .select('t2.*')
            .then(result => {
              expect(result).to.have.length(3);
    ...
    describe('QueryBuilder JSON queries', () => {
      ...
      before(() => {
        ...
        complexJsonObj.jsonObject.jsonArray = _.cloneDeep(
          complexJsonObj.jsonArray
        );
      ...
      it('should have test data', () => {
        return BoundModel.query().then(all => {
          expect(
            _.find(all, {name: 'complex line'}).jsonObject.stringField
          ).to.be(complexJsonObj.jsonObject.stringField);
        });
      });
      ...
      describe('.whereJsonEquals(fieldExpr, <array|object|string>)', () => {
        ...
        it('should find results jsonArray[0].arrayField[0] == { noMoreLevels: true }', () => {
          return BoundModel.query()
            .whereJsonEquals('jsonArray:[0].arrayField[0]', {
              noMoreLevels: true
            })
            .then(results => {
        ...
        it('should find results jsonArray[0].arrayField[0] == { noMoreLevels: true }', () => {
          return BoundModel.query()
            .where(
              ref('jsonArray:[0].arrayField[0]'),
              lit({noMoreLevels: true})
            )
            .then(results => {
              expectIdsEqual(results, [1]);
            });
        ...
        it('should find results with jsonArray == complexJsonObj.jsonArray', () => {
          return BoundModel.query()
            .whereJsonEquals('jsonArray', complexJsonObj.jsonArray)
            .then(results => {
              expectIdsEqual(results, [1]);
            });
        });
        ...
        it('should find results with jsonObject,jsonArray == complexJsonObj.jsonArray', () => {
          return BoundModel.query()
            .whereJsonEquals('jsonObject:jsonArray', complexJsonObj.jsonArray)
            .then(results => {
              expectIdsEqual(results, [1]);
            });
        });
      ...
      describe('.whereJsonSupersetOf(fieldExpr, <array|object|string>)', () => {
        ...
        it('should find subset where both sides are references', () => {
          return BoundModel.query()
            .where(
              ref('jsonObject:a').castJson(),
              '@>',
              ref('jsonObject:b').castJson()
            )
            .then(results => {
              expectIdsEqual(results, [7]);
            });
        ...
        it('should find results jsonObject.objectField @> complexJsonObj.jsonObject.objectField', () => {
          return BoundModel.query()
            .whereJsonSupersetOf(
              ...
              complexJsonObj.jsonObject.objectField
            )
            .then(results => {
              expectIdsEqual(results, [1]);
            });
        ...
        it('should not find results NOT(jsonObject.x @> jsonObject.y)', () => {
          return BoundModel.query()
            .whereNot(
              ref('jsonObject:x').castJson(),
              '@>',
              ref('jsonObject:y').castJson()
            )
            .then(results => {
              expectIdsEqual(results, []);
            });
      ...
      describe('.whereJsonHasAny(fieldExpr, keys) and .whereJsonHasAll(fieldExpr, keys)', () => {
        ...
        it('should find results for a and b', () => {
          ...
          return BoundModel.query()
            .where(
              raw('?? \\?& ?', ['jsonObject', lit(['a', 'b']).castArray()])
            )
            .then(results => {
              expectIdsEqual(results, [7]);
            });
      ...
      describe('.whereJsonField(fieldExpr, operator, value)', () => {
        it('should throw error if operator is not valid', () => {
          expect(() => {
            BoundModel.query().whereJsonField(
        ...
        it('should be able to find numbers with >', () => {
          return BoundModel.query()
            .whereJsonField('jsonObject:numberField', '>', 1.4)
            .then(results => {
              expectIdsEqual(results, [1]);
            });
        });
        ...
        it('should be able to find numbers with >', () => {
          return BoundModel.query()
            .where(
              ref('jsonObject:numberField').castFloat(),
              '>',
              lit(1.4).castFloat()
            )
            .then(results => {
        ...
        it('should not find where 1.5 < 1.5', () => {
          return BoundModel.query()
            .whereJsonField('jsonObject:numberField', '<', 1.5)
            .then(results => {
              expectIdsEqual(results, []);
            });
        });
        ...
        it('should be able to find strings with !=', () => {
          return BoundModel.query()
            .whereJsonField('jsonObject:stringField', '!=', 'not me')
            .then(results => {
              expectIdsEqual(results, [1]);
            });
        });
        ...
        it('should be able to find strings with like', () => {
          return BoundModel.query()
            .whereJsonField('jsonObject:stringField', 'LIKE', '%jsonObject.st%')
            .then(results => {
              expectIdsEqual(results, [1]);
            });
        });
        ...
        it('should be able to find is not null', () => {
          return BoundModel.query()
            .whereJsonField('jsonObject:stringField', 'IS NOT', null)
            .then(results => {
              expectIdsEqual(results, [1]);
            });
        });
        ...
        it('should be able to find boolean equals false', () => {
          return BoundModel.query()
            .whereJsonField('jsonObject:booleanField', '=', false)
            .then(results => {
              expectIdsEqual(results, [1]);
            });
        });
        ...
        it('should be able to find every but first row where booleanField equals true or is NULL', () => {
          return BoundModel.query()
            .where(
              ref('jsonObject:booleanField').castBool(),
              '=',
              lit(true).castBool()
            )
            .orWhere(ref('jsonObject:booleanField'), 'IS', null)
