  ...
  describe('#ref{id.prop}', () => {
    it('should replace references with the inserted values (1)', () => {
      let models = [
        {
          firstName:
            'I am the child of #ref{parent.firstName} #ref{parent.lastName}',
  ...
  function test(opt) {
    ...
    function createInserter() {
      ...
      if (opt.allowedRelations) {
        insertOpt.allowedRelations = RelationExpression.parse(
          opt.allowedRelations
        );
      }
    ...
    return inserter
      .execute(tableInsertion => {
        ...
        _.each(tableInsertion.models, (model, idx) => {
          if (_.isArray(opt.models)) {
            expect(opt.models.indexOf(model) !== -1).to.equal(
              tableInsertion.isInputModel[idx]
            );
          } else {
            expect(model === opt.models).to.equal(
              tableInsertion.isInputModel[idx]
            );
          }
        });
