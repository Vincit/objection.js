...
describe('ManyToManyRelation', () => {
  ...
  beforeEach(() => {
    ...
    compositeKeyRelation = new ManyToManyRelation(
      'nameOfOurRelation',
      OwnerModel
    );
  ...
  it('should accept a join model in join.through object', () => {
    ...
    expect(
      classUtils.isSubclassOf(relation.joinModelClass(mockKnex), JoinModel)
    ).to.equal(true);
  });
  ...
  it('should accept an absolute file path to a join model in join.through object', () => {
    ...
    expect(
      classUtils.isSubclassOf(
        relation.joinModelClass(mockKnex),
        require('./files/JoinModel')
      )
    ).to.equal(true);
  });
  ...
  it('should accept a composite keys in join.through object (1)', () => {
    ...
    expect(relation.joinTable).to.equal('JoinModel');
    expect(relation.joinTableOwnerProp.cols).to.eql([
      'ownerName',
      'ownerDateOfBirth'
    ]);
    expect(relation.joinTableRelatedProp.cols).to.eql(['relatedId']);
  ...
  describe('insert', () => {
    ...
    it('should generate an insert query (composite key)', () => {
      mockKnexQueryResults = [
        [{id: 1, cid: 33, did: 44}, {id: 2, cid: 33, did: 55}]
      ];
