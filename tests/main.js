...
describe('main module', () => {
  it('should be able to load using require', () => {
    ...
    expect(objection.QueryBuilder).to.equal(
      require('../lib/queryBuilder/QueryBuilder')
    );
    expect(objection.QueryBuilderBase).to.equal(
      require('../lib/queryBuilder/QueryBuilderBase')
    );
    ...
    expect(objection.ValidationError).to.equal(
      require('../lib/model/ValidationError')
    );
    expect(objection.NotFoundError).to.equal(
      require('../lib/model/NotFoundError')
    );
    expect(objection.Relation).to.equal(require('../lib/relations/Relation'));
    expect(objection.HasManyRelation).to.equal(
      require('../lib/relations/hasMany/HasManyRelation')
    );
    expect(objection.HasOneRelation).to.equal(
      require('../lib/relations/hasOne/HasOneRelation')
    );
    ...
    expect(objection.transaction.start).to.equal(
      require('../lib/transaction').start
    );
    expect(objection.ref).to.equal(
      require('../lib/queryBuilder/ReferenceBuilder').ref
    );
    expect(objection.raw).to.equal(
      require('../lib/queryBuilder/RawBuilder').raw
    );
    expect(objection.lit).to.equal(
      require('../lib/queryBuilder/LiteralBuilder').lit
    );
    expect(objection.Promise).to.equal(require('bluebird'));
    expect(objection.Validator).to.equal(require('../lib/model/Validator'));
    expect(objection.AjvValidator).to.equal(
      require('../lib/model/AjvValidator')
    );
