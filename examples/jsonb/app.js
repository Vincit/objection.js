...
.then(function (data) {
  ...
  return Promise.each([
    testQuery(
      "db.Place.query().whereJsonIsObject('JustTable.imaColumn:the.same.sql.generated')", {justDumpSql: true}
    ),
    testQuery(
      "db.Place.query().whereJsonIsObject('JustTable.imaColumn:[the][same][sql][generated]')", {justDumpSql: true}
    ),
    testQuery(
      "db.Hero.query().select('name').whereJsonHasAny('details', ['title', 'race'])"
    ),
    testQuery(
      "db.Hero.query().select('name').whereJsonHasAll('details', ['title', 'race'])"
    ),
    testQuery(
      "db.Hero.query().select('name').whereJsonHasAny('details', ['0'])"
    ),
    ...
    // field type object
    testQuery(
      "db.Hero.query().select('name').whereJsonIsObject('details')"
    ),
    // field type array
    testQuery(
      "db.Hero.query().select('name').whereJsonIsArray('details')"
    ),
