...
module.exports = function transaction() {
  ...
  if (arguments.length < 2) {
    return Promise.reject(new Error('moron.transaction: provide at least one Model class to bind to the transaction'));
  }
  ...
  for (i = 0; i < modelClasses.length; ++i) {
    if (!utils.isSubclassOf(modelClasses[i], Model)) {
      return Promise.reject(new Error('moron.transaction: all but the last argument should be Model subclasses'));
    }
  }
  ...
  for (i = 0; i < modelClasses.length; ++i) {
    if (modelClasses[i].knex() !== knex) {
      return Promise.reject(new Error('moron.transaction: all Model subclasses must be bound to the same database'));
    }
  }
