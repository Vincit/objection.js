...
export default function queryBuilderMethod(input, name) {
...
function normalizeInput(input) {
  if (_.isFunction(input) || _.isArray(input)) {
    return {
      default: normalizeQueryMethodDesc(input)
    };
  } else {
    return _.mapValues(input, normalizeQueryMethodDesc);
  }
}

function normalizeQueryMethodDesc(methodDesc) {
