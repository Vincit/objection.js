...
MoronModelBase.prototype.$setJson = function (json, options) {
  ...
  if (!options.patch) {
    json = this.constructor.$$mergeWithDefaults(json);
  }
  ...
  for (var key in json) {
    if (this.constructor.hasOwnJsonProperty(json, key)) {
      this[key] = json[key];
    }
  }
};
...
MoronModelBase.propertyNameToColumnName = function (propertyName) {
  var model = new this();
  var value = uuid.v4();

  model[propertyName] = value;

  var row = model.$toDatabaseJson();
