...
// TODO: This could be moved inside the `.pug` code.
function preprocessParsedExpression(parsedExpr) {
  parsedExpr.columnParts = parsedExpr.columnName.split('.').map(part => part.trim());
  parsedExpr.column = parsedExpr.columnParts[parsedExpr.columnParts.length - 1];

  if (parsedExpr.columnParts.length >= 2) {
    parsedExpr.table = parsedExpr.columnParts.slice(0, parsedExpr.columnParts.length - 1).join('.');
  } else {
...
function freezeParsedExpr(parsedExpr) {
  ...
  Object.freeze(parsedExpr.columnParts);
