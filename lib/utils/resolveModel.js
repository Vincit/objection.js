...
const getModel = memoize(() => require('../model/Model'));
...
function resolveModel(modelRef, modelPaths, errorPrefix) {
  ...
  if (typeof modelRef === 'string') {
    ...
    if (!isSubclassOf(modelRef, Model)) {
      throw new Error(`${errorPrefix} is not a subclass of Model or a file path to a module that exports one.`);
    }
  ...
  if (isSubclassOf(mod, Model)) {
    ...
    Object.keys(mod).forEach(exportName => {
      ...
      if (isSubclassOf(exp, Model)) {
        if (ModelClass !== null) {
          throw new Error(`${errorPrefix} path ${path} exports multiple models. Don't know which one to choose.`);
        }
