  ...
  return {
    ...
    postProcessResponse(result) {
      ...
      if (Array.isArray(normalizedResult)) {
        const output = new Array(normalizedResult.length);

        for (let i = 0, l = normalizedResult.length; i < l; ++i) {
          output[i] = parseKeys(normalizedResult[i]);
        }

        return output;
      } else {
        return parseKeys(normalizedResult);
      }
    }
  };
}
