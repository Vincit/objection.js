...
ManyToManyRelation.prototype.findQuery = function (builder, id, isColumnRef) {
  ...
  if (isColumnRef) {
    ...
    builder.whereRaw(formatter.wrap(this.fullJoinTableOwnerCol()) + ' = ' + formatter.wrap(id));
  } else {
    if (_.isArray(id)) {
      builder.whereIn(this.fullJoinTableOwnerCol(), id);
    } else {
      builder.where(this.fullJoinTableOwnerCol(), id);
    }
  }
