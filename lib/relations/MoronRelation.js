...
MoronRelation.prototype.setMapping = function (mapping) {
  ...
  if (mapping.join.through) {
    ...
    if (joinFrom.table === this.ownerModelClass.tableName) {
      this.joinTableOwnerCol = joinTableFrom.table + '.' + joinTableFrom.name;
      this.joinTableRelatedCol = joinTableTo.table + '.' + joinTableTo.name;
    } else {
      this.joinTableRelatedCol = joinTableFrom.table + '.' + joinTableFrom.name;
      this.joinTableOwnerCol = joinTableTo.table + '.' + joinTableTo.name;
    }
  }

  this.query = parseMappingQuery(mapping);
  this.ownerProp = this._propertyName(joinOwner, this.ownerModelClass);
  this.ownerCol = joinOwner.table + '.' + joinOwner.name;
  this.relatedProp = this._propertyName(joinRelated, this.relatedModelClass);
  this.relatedCol = joinRelated.table + '.' + joinRelated.name;
};
