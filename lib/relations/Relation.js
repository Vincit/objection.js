...
class Relation {
  ...
  createError(message) {
    if (this.ownerModelClass && this.ownerModelClass.name && this.name) {
      return new Error(
        `${this.ownerModelClass.name}.relationMappings.${this.name}: ${message}`
      );
    } else {
...
function createJoinProperties(ctx) {
  ...
  if (fromProp.modelClass.tableName === ctx.ownerModelClass.tableName) {
    ...
    throw ctx.createError(
      'join: either `from` or `to` must point to the owner model table.'
    );
  }

  if (relatedProp.modelClass.tableName !== ctx.relatedModelClass.tableName) {
    throw ctx.createError(
      'join: either `from` or `to` must point to the related model table.'
    );
  }
...
function createRelationProperty(ctx, refString, propName) {
  try {
    return new RelationProperty(refString, table => {
      return [ctx.ownerModelClass, ctx.relatedModelClass].find(
        it => it.tableName === table
      );
    });
  } catch (err) {
    if (err instanceof RelationProperty.ModelNotFoundError) {
      throw ctx.createError(
        'join: either `from` or `to` must point to the owner model table and the other one to the related table.'
      );
    } else {
      throw ctx.createError(
