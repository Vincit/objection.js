...
class Relation {
  ...
  join(
    builder,
    { joinOperation, relatedTableAlias, relatedJoinSelectQuery, relatedTable, ownerTable } = {}
  ) {
    const relModelClass = this.relatedModelClass;
    const ownModelClass = this.ownerModelClass;
    ...
    if (!relatedJoinSelectQuery) {
      relatedJoinSelectQuery = relModelClass.query().childQueryOf(builder);
    }

    if (!relatedTable) {
      relatedTable = builder.tableNameFor(relModelClass);
    }

    if (!ownerTable) {
      ownerTable = builder.tableRefFor(ownModelClass);
    }
