...
function createJoinProperties(ctx) {
  ...
  if (ownerProp.props.some(it => it === ctx.name)) {
    throw ctx.createError(
      `join: relation mapping name and join property '${ctx.name}' cannot have the same name. If you cannot change one or the other, you can use $parseDatabaseJson and $formatDatabaseJson methods to convert the column name.`
    );
  }
