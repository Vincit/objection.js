  ...
  resolveModel(Model, modelClass, logPrefix) {
    ...
    if (typeof modelClass === 'string') {
      ...
      if (isAbsolutePath(modelClass)) {
        ...
        this.ownerModelClass.modelPaths.forEach(modelPath => {
      ...
      if (!isSubclassOf(ModelClass, Model)) {
        throw this.createError(`${logPrefix}: ${modelClass} is an invalid file path to a model class`);
      }

      return ModelClass;
    } else {
      if (!isSubclassOf(modelClass, Model)) {
        throw this.createError(`${logPrefix} is not a subclass of Model or a file path to a module that exports one.`);
      }
