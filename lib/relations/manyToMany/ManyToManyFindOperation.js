...
class ManyToManyFindOperation extends RelationFindOperation {
  constructor(name, opt) {
    ...
    this.ownerJoinColumnAlias = new Array(
      this.relation.joinTableOwnerProp.size
    );
  ...
  onBuild(builder) {
    ...
    if (!builder.has(builder.constructor.SelectSelector)) {
      ...
      for (let i = 0, l = this.relation.joinTableExtras.length; i < l; ++i) {
        ...
        builder.select(
          `${joinTable}.${extra.joinTableCol} as ${extra.aliasCol}`
        );
      }
    }
    ...
    for (let i = 0, l = joinTableOwnerProp.size; i < l; ++i) {
      ...
      const propName = relatedModelClass.columnNameToPropertyName(
        this.ownerJoinColumnAlias[i]
      );
