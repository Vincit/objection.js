...
class ManyToManyRelation extends Relation {
  ...
  join(builder, opt) {
    ...
    opt.relatedTableAlias =
      opt.relatedTableAlias || this.relatedTableAlias(builder);
    opt.relatedJoinSelectQuery =
      opt.relatedJoinSelectQuery ||
      this.relatedModelClass.query().childQueryOf(builder);
    opt.relatedTable =
      opt.relatedTable || builder.tableNameFor(this.relatedModelClass);
    opt.ownerTable =
      opt.ownerTable || builder.tableRefFor(this.ownerModelClass);
    ...
    return builder[opt.joinOperation](joinTableAsAlias, join => {
      ...
      for (let i = 0, l = relatedProp.size; i < l; ++i) {
        const joinTableRelatedRef = joinTableRelatedProp
          .ref(builder, i)
          .table(opt.joinTableAlias);
        const relatedRef = relatedProp
          .ref(builder, i)
          .table(opt.relatedTableAlias);
  ...
  createJoinModels(ownerId, related) {
    ...
    for (let i = 0, lr = related.length; i < lr; ++i) {
      ...
      for (let j = 0, lp = this.joinTableRelatedProp.size; j < lp; ++j) {
        this.joinTableRelatedProp.setProp(
          joinModel,
          j,
          this.relatedProp.getProp(rel, j)
        );
      }
...
function checkThroughObject(ctx) {
  ...
  if (!mapping.join.through || typeof mapping.join.through !== 'object') {
    throw ctx.createError(
      'join must have a `through` object that describes the join table.'
    );
  }
...
function createJoinProperties(ctx) {
  ...
  ret = createRelationProperty(
    ctx,
    ctx.mapping.join.through.from,
    'join.through.from'
  );
  fromProp = ret.prop;
  ctx = ret.ctx;
  ...
  ret = createRelationProperty(
    ctx,
    ctx.mapping.join.through.to,
    'join.through.to'
  );
  toProp = ret.prop;
  ctx = ret.ctx;

  if (fromProp.modelClass.tableName !== toProp.modelClass.tableName) {
    throw ctx.createError(
      'join.through `from` and `to` must point to the same join table.'
    );
  }

  if (ctx.relatedProp.modelClass.tableName === fromProp.modelClass.tableName) {
