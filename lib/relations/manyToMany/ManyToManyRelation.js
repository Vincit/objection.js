...
function createRelationProperty(ctx, refString, messagePrefix) {
  ...
  try {
    ...
    if (err instanceof RelationProperty.ModelNotFoundError) {
      ...
      throw ctx.createError(`${messagePrefix} must have format JoinTable.columnName. For example "JoinTable.someId" or in case of composite key ["JoinTable.a", "JoinTable.b"].`);
    }
  }
