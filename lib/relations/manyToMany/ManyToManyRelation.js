...
class ManyToManyRelation extends Relation {
  ...
  findQuery(builder, opt) {
    ...
    const joinTableAsAlias = `${joinTable} as ${joinTableAlias}`;

    builder.join(joinTableAsAlias, join => {
...
function defaultJoinOperation(relation, builder) {
  return 'join';
}

function defaultRelatedTablealias(relation, builder) {
  return relation.relatedTableAlias(builder);
}
