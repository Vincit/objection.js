...
/**
 * Shortcut for `Model.traverse(this, callback)`.
 *
 * @param {Function(Model)} callback
 */
Model.prototype.$traverse = function (callback) {
  return this.constructor.traverse(this, callback);
};
 ...
 * @param {Function(Model)} callback
 */
Model.traverse = function (models, callback) {
  traverse(models, callback);
};
...
function traverse(models, callback) {
  ...
  if (_.isArray(models)) {
    for (var i = 0, l = models.length; i < l; ++i) {
      traverseOne(models[i], callback);
    }
  } else {
    traverseOne(models, callback)
  }
}
 ...
 * @private
 */
function traverseOne(model, callback) {
  ...
  for (var relName in model.constructor.getRelations()) {
    if (_.has(model, relName)) {
      traverse(model[relName], callback);
    }
  }
}
