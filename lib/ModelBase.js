...
ModelBase.prototype.$$toJson = function (createDbJson) {
  var json = toJsonImpl(this, createDbJson);
 ...
 * @private
 */
function toJsonImpl(self, createDbJson) {
  var json = {};

  _.each(self, function toJsonImplLooper(value, key) {
    if (key.charAt(0) !== '$' && !_.isFunction(value) && !_.isUndefined(value)) {
      if (_.isObject(value)) {
...
function toJsonObject(value, createDbJson) {
  if (_.isArray(value)) {
    ...
    return value.$$toJson(createDbJson);
  } else {
