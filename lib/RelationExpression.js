 ...
 * @returns {RelationExpression}
 */
RelationExpression.prototype.args = function (relationName) {
  for (var i = 0, l = this.nodes.length; i < l; ++i) {
    var node = this.nodes[i];
 ...
 * @constructor
 */
function RelationExpressionNode(name, args) {
  this.name = name;
  this.args = args || [];
  this.children = [];
}
...
RelationExpressionParser.prototype._parseToken = function (token, nodes) {
  ...
  if (parts) {
    ...
    args = parts[2].replace(/\s/g, '').split(',');
  ...
  var node = new RelationExpressionNode(name, args);
  nodes.push(node);
