...
MoronModel.prototype.$query = function (transaction) {
  ...
  return MoronQueryBuilder
    ...
    .findImpl(function () {
      this.where(ModelClass.getIdColumn(), self.$id());
    })
    .insertImpl(function () {
      ModelClass.$$insert(this, self);
    })
    .updateImpl(function (update) {
      ModelClass.$$update(this, update || self).where(ModelClass.getIdColumn(), self.$id());
    })
    .patchImpl(function (patch) {
      ModelClass.$$patch(this, patch || self).where(ModelClass.getIdColumn(), self.$id());
    })
    .deleteImpl(function () {
      ModelClass.$$delete(this).where(ModelClass.getIdColumn(), self.$id());
    })
    .relateImpl(function () {
...
MoronModel.getIdColumn = function () {
  if (!this.$$idColumn) {
    var idColumn = this.propertyNameToColumnName(this.idProperty);

    if (!idColumn) {
      throw new Error(this.name +
        '.$formatDatabaseJson probably changes the value of the id property `' + this.idProperty +
        '` which is a no-no.');
    }

    this.$$idColumn = this.tableName + '.' + idColumn;
  }
...
MoronModel.loadRelated = function ($models, expression, transaction) {
  ...
  if (_.isString(expression)) {
    expression = MoronEagerExpression.parse(expression);
  }
...
MoronModel.$$insert = function (builder, $models) {
  ...
  return builder.insert(json).returning(ModelClass.getIdColumn()).runAfterModelCreatePushFront(function (ids) {
    // TODO
