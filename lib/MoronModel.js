...
MoronModel.prototype.$query = function (transaction) {
  ...
  return MoronQueryBuilder
    ...
    .findImpl(function () {
      this.where(idColumn, self.$id());
    })
    .insertImpl(function () {
      ModelClass.$$insert(this, self);
    })
    .updateImpl(function (update) {
      ModelClass.$$update(this, update || self).where(idColumn, self.$id());
    })
    .patchImpl(function (patch) {
      ModelClass.$$patch(this, patch || self).where(idColumn, self.$id());
    })
    .deleteImpl(function () {
...
MoronModel.getRelations = function () {
  ...
  if (!this.$$relations) {
    this.$$relations = _.reduce(this.getRelationMappings(), function (relations, mapping, relationName) {
      relations[relationName] = new mapping.relation(relationName, mapping, ModelClass);
      return relations;
    }, Object.create(null));
  }
...
MoronModel.getRelationMappings = function () {
  ...
  return _.reduce(_.result(this, 'relationMappings'), function (mappings, mapping, relationName) {
