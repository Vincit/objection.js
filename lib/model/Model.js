...
const parseJsonAttributes = require('./modelJsonAttributes')
  .parseJsonAttributes;
const formatJsonAttributes = require('./modelJsonAttributes')
  .formatJsonAttributes;
const idColumnToIdProperty = require('./modelColPropMap').idColumnToIdProperty;
const columnNameToPropertyName = require('./modelColPropMap')
  .columnNameToPropertyName;
const propertyNameToColumnName = require('./modelColPropMap')
...
class Model {
  ...
  static propertyNameToColumnName(propertyName) {
    ...
    if (!this.$$propToCol[propertyName]) {
      this.$$propToCol[propertyName] = propertyNameToColumnName(
        this,
        propertyName
      );
    }
...
function getIdRelationProperty(ModelClass) {
  const idColumn = Array.isArray(ModelClass.idColumn)
    ? ModelClass.idColumn
    : [ModelClass.idColumn];

  return new RelationProperty(
    idColumn.map(idCol => {
      return `${ModelClass.tableName}.${idCol}`;
    }),
    () => ModelClass
  );
}
