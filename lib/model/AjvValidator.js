...
function deepRemoveRequired(jsonSchemaProperties) {
  if (jsonSchemaProperties === undefined) {
    return;
  }

  if (jsonSchemaProperties.required !== undefined) {
    delete jsonSchemaProperties.required;
  }

  if (jsonSchemaProperties.type !== 'object' || jsonSchemaProperties.properties === undefined) {
    return;
  }

  for (const key of Object.keys(jsonSchemaProperties.properties)) {
    deepRemoveRequired(jsonSchemaProperties.properties[key]);
  }
}
...
function jsonSchemaWithoutRequired(jsonSchema) {
  ...
  const clonedSchema = Object.assign(jsonSchema, {});
  deepRemoveRequired(clonedSchema);

  return Object.assign(
    omit(clonedSchema, subSchemaProps),
    ...subSchemaProps.map(prop => subSchemaWithoutRequired(clonedSchema, prop))
  );
}
