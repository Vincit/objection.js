...
class AjvValidator extends Validator {
  ...
  compilePatchValidator(jsonSchema) {
    let clonedSchema = cloneDeep(jsonSchema);
    clonedSchema = jsonSchemaWithoutRequired(clonedSchema);
    // We need to use the ajv instance that doesn't set the default values.
    return this.ajvNoDefaults.compile(clonedSchema);
  }
...
function deepRemoveRequired(jsonSchema) {
  if (jsonSchema === undefined || jsonSchema.properties === undefined) {
    return;
  }

  for (const prop of Object.keys(jsonSchema.properties)) {
    const fieldDef = jsonSchema.properties[prop];

    if (fieldDef.type === 'object') {
      jsonSchema.properties[prop] = jsonSchemaWithoutRequired(fieldDef);
    } else if (
      fieldDef.type === 'array' &&
      fieldDef.items &&
      ...
      jsonSchema.properties[prop].items = jsonSchemaWithoutRequired(fieldDef.items);
    }
  }
}
