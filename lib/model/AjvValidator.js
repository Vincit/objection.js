...
function jsonSchemaWithoutRequired(jsonSchema) {
  ...
  if (rtn.properties !== undefined) {
    for (const prop of Object.keys(rtn.properties)) {
      const fieldDef = rtn.properties[prop];

      if (fieldDef.type === 'object') {
        rtn.properties[prop] = jsonSchemaWithoutRequired(fieldDef);
      } else if (
        fieldDef.type === 'array' &&
        fieldDef.items &&
        ...
        rtn.properties[prop].items = jsonSchemaWithoutRequired(fieldDef.items);
      }
    }
  }

  return omit(rtn, ['required', ...subSchemaProps]);
}
