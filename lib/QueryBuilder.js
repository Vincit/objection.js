 ...
 * should execute some other query.
 *
...
QueryBuilder.prototype.isFindQuery = function () {
  return !this._calledWriteMethod && !_.any(this._knexMethodCalls, function (call) {
    return /insert|update|delete/.test(call.method);
  });
};
 ...
 * @ignore
 */
QueryBuilder.prototype.copyFrom = function (queryBuilder, regex) {
  var self = this;

  _.each(queryBuilder._knexMethodCalls, function (call) {
    if (!regex || regex.test(call.method)) {
...
QueryBuilder.prototype._execute = function () {
  ...
  return promise.then(function () {
    // We need to build the builder even if the _explicitRejectValue or _hooks.executor
...
QueryBuilder.prototype.insertWithRelated = tryCallWriteMethod(function insertWithRelated(modelsOrObjects) {
  ...
  this.setQueryExecutor(function (builder) {
    ...
    return inserter.execute(function (tableInsertion) {
      ...
      if (tableInsertion.modelClass) {
        insertQuery = tableInsertion.modelClass.query();
      ...
      return Promise.all(_.flatten([
        batchInsert(inputs, tableInsertion.tableName, insertQuery && insertQuery.copyFrom(builder, /returning/)),
        batchInsert(others, tableInsertion.tableName, insertQuery)
      ]));
    });
  });
  ...
  function batchInsert(models, tableName, queryBuilder) {
    ...
    if (queryBuilder) {
      ...
      return _.map(batches, function (batch) {
        return ModelClass.knex().insert(batch).into(tableName);
      });
    }
  }
});
...
QueryBuilder.prototype.$$updateWithOptions = tryCallWriteMethod(function $$updateWithOptions(modelOrObject, method, opt, fetchId) {
  ...
  this.runAfterModelCreate(function (numUpdated) {
    ...
    return promise.then(function (result) {
      return [result, update.model().$afterUpdate(opt)];
...
function eagerFetch(builder, models) {
  if (models instanceof builder._modelClass || (_.isArray(models) && models[0] instanceof builder._modelClass)) {
    return builder._modelClass.loadRelated(models, builder._eagerExpression, builder._eagerFilters);
  } else {
    return models;
  }
}
