 ...
 * a query builder. You can bind `Model` classes to the transaction using the `Model.bindKnex`
 ...
 *   transaction = trx;
 *   return Person.bindKnex(transaction).query().insert({firstName: 'Jennifer'});
 * }).then(function (jennifer) {
 *   return Person.bindKnex(transaction).query().patch({lastName: 'Lawrence'}).where('id', jennifer.id);
 * }).then(function () {
