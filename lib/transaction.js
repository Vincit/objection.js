...
module.exports.start = function start(modelBase) {
  if (arguments.length < 1) {
    return Promise.reject(new Error('objection.transaction.start: provide at least one Model class to bind to the transaction'))
  }

  if (!utils.isSubclassOf(modelBase, Model)) {
    return Promise.reject(new Error('objection.transaction.start: all but the last argument should be Model subclasses'));
  }

  var knex = modelBase.knex();


  return new Promise(function(resolve) {
    knex.transaction(function (trx) {
        modelBase = modelBase.knex(trx);

      resolve({
        __trx: trx,
        commit: function() {
          return this.__trx.commit();
        },
        rollback: function() {
          return this.__trx.rollback();
        }
      });
    });
