  ...
  build(modelClass, models) {
    ...
    if (Array.isArray(models)) {
      for (let i = 0, l = models.length; i < l; ++i) {
        this.buildForModel(modelClass, models[i], null, null, this.allowedRelations, null);
      }
    } else {
      this.buildForModel(modelClass, models, null, null, this.allowedRelations, null);
    }
  ...
  buildForRelations(modelClass, node, allowedRelations, dataPath) {
    ...
    for (let i = 0, l = relations.length; i < l; ++i) {
      ...
      if (relModels) {
        ...
        const relPath = getDataPath(dataPath, rel.name);
        if (Array.isArray(relModels)) {
          for (let i = 0, l = relModels.length; i < l; ++i) {
            this.buildForModel(
              ...
              rel,
              nextAllowed,
              getDataPath(relPath, i)
            );
          }
        } else {
          this.buildForModel(rel.relatedModelClass, relModels, node, rel, nextAllowed, relPath);
        }
      }
    }
  }
