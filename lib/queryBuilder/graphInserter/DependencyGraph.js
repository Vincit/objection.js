  ...
  buildForModel(modelClass, model, parentNode, rel, allowedRelations) {
    ...
    if (rel) {
      if (rel instanceof HasManyRelation) {
        ...
        parentNode.manyToManyConnections.push(
          new ManyToManyConnection(node, rel)
        );
      }
    }
      ...
      if (ref) {
        ...
        if (!actualNode) {
          throw refNode.modelClass.createValidationError({
            ref: `could not resolve reference "${ref}"`
          });
        }
        ...
        for (
          let m = 0, lm = refNode.manyToManyConnections.length;
          m < lm;
          ++m
        ) {
          actualNode.manyToManyConnections.push(
            refNode.manyToManyConnections[m]
          );
        }
        ...
        allMatches(propRefRegex, value, matchResult => {
          ...
          if (!refNode) {
            throw node.modelClass.createValidationError({
              ref: `could not resolve reference "${value}"`
            });
          }
          ...
          if (value === match) {
            ...
            // (date, number, etc).
            node.needs.push(
              new ReplaceValueDependency(refNode, path, refProp, false)
            );
            refNode.isNeededBy.push(
              new ReplaceValueDependency(node, path, refProp, true)
            );
          } else {
            // If the match is inside a string, replace the reference inside the string with
            // the resolved value.
            node.needs.push(
              new InterpolateValueDependency(
                refNode,
                path,
                refProp,
                match,
