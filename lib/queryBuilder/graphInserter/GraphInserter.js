...
class GraphInserter {
  ...
  _beforeInsertBatch(batch, executorMethod) {
    ...
    for (let i = 0, lt = tableNames.length; i < lt; ++i) {
      ...
      for (let j = 0, li = tableInsertion.items.length; j < li; ++j) {
        const model = tableInsertion.items[j].model;
        const relation = tableInsertion.items[j].relation;
    ...
    modelsByRelation.forEach((models, relation) => {
      promises.push(relation[executorMethod](models, this.queryContext, null));
    });
