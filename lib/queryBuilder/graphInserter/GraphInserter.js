  ...
  _createJoinRowBatch() {
    ...
    for (let n = 0, ln = this.graph.nodes.length; n < ln; ++n) {
      ...
      for (let m = 0, lm = node.manyToManyConnections.length; m < lm; ++m) {
        ...
        let joinModel = conn.relation.createJoinModels(ownerProp, [
          conn.node.model
        ])[0];
        ...
        if (conn.refNode) {
          ...
          for (
            let k = 0, lk = conn.relation.joinTableExtras.length;
            k < lk;
            ++k
          ) {
            ...
            if (conn.refNode.model[extra.aliasProp] !== undefined) {
              joinModel[extra.joinTableProp] =
                conn.refNode.model[extra.aliasProp];
            }
          }
        }
