...
class GraphInserter {
  ...
  _executeNextBatch(inserter) {
    ...
    return this._beforeInsertBatch(batch)
      .then(() => {
        // Insert the batch one table at a time.
        return Promise.all(
          Object.keys(batch).map(tableName => {
  ...
  _beforeInsertBatch(batch) {
    const tableNames = Object.keys(batch);
    const modelsByRelation = new Map();

    for (let i = 0, lt = tableNames.length; i < lt; ++i) {
      const tableName = tableNames[i];
      const tableInsertion = batch[tableName];
      const uidProp = tableInsertion.modelClass.uidProp;

      for (let j = 0, lm = tableInsertion.models.length; j < lm; ++j) {
        const model = tableInsertion.models[j];
        const node = this.graph.nodesById[model[uidProp]];

        if (node.relation) {
          let relModels = modelsByRelation.get(node.relation);

          if (relModels === undefined) {
            relModels = [];
            modelsByRelation.set(node.relation, relModels);
          }
    ...
    modelsByRelation.forEach((models, relation) => {
      promises.push(relation.executeBeforeInsert(models, this.queryContext, null));
    });
  ...
  _markBatchInserted(batch) {
    ...
    for (let t = 0, lt = tableNames.length; t < lt; ++t) {
      ...
      for (let m = 0, lm = models.length; m < lm; ++m) {
        nodes[models[m][modelClass.uidProp]].markAsInserted();
      }
    }
  }
