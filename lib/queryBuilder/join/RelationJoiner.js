...
class RelationJoiner {
  ...
  build(builder, buildSelects = true) {
    const tableTree = this._getTableTree(builder);
    const userSelectQueries = new Map([[tableTree.rootNode, builder]]);

    for (const tableNode of tableTree.rootNode.childNodes) {
      this._buildJoinsForNode({ builder, tableNode, userSelectQueries });
    }

    if (buildSelects) {
      this._buildSelects({ builder, tableNode: tableTree.rootNode, userSelectQueries });
    }
  }
  ...
  parseResult(builder, flatRows) {
    const parser = JoinResultParser.create({
      ...
      omitColumnAliases: this.internalSelections.map(it => it.name)
    });
  ...
  _buildJoinsForNode({ builder, tableNode, userSelectQueries }) {
    ...
    // relation.join also applies the relation modifier that can
...
function containsSelection(builder, selections, selection) {
  return selections.some(it => Selection.doesSelect(builder, it, selection));
}
...
function selectionsToStrings(selections) {
  return selections.map(selection => {
    ...
    return selection.alias !== selection.column ? `${selectStr} as ${selection.alias}` : selectStr;
  });
}
