...
function fetchCurrentGraph(builder, graphOptions, obj) {
  if (graphOptions.isInsertOnly()) {
    ...
    return GraphOperation.fetchCurrentGraph({ builder, obj });
  }
}
...
function checkForErrors(graph, graphOptions, builder) {
  return currentGraph => {
    checkForNotFoundErrors(graph, currentGraph, graphOptions);
...
function checkForNotFoundErrors(graph, currentGraph, graphOptions) {
  for (const node of graph.nodes) {
    if (
      ...
      if (!node.parentNode) {
        throw node.modelClass.createNotFoundError({
