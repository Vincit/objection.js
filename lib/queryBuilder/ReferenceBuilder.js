...
class ReferenceBuilder extends LazyBuilder {
  constructor(expr) {
    super();
  ...
  get isPlainColumnRef() {
    return (
      this._reference.access.length === 0 &&
      !this._cast &&
      !this._toJson &&
      !this._as
    );
  ...
  toRawArgs() {
    ...
    if (this._reference.access.length > 0) {
      ...
      let jsonFieldRef = this._reference.access
        .map(field => field.ref)
        .join(',');
      referenceSql = `??${extractor}'{${jsonFieldRef}}'`;
    }

    let castedRefQuery = this._cast
      ? `CAST(${referenceSql} AS ${this._cast})`
      : referenceSql;
    let toJsonQuery = this._toJson
      ? `to_jsonb(${castedRefQuery})`
      : castedRefQuery;
