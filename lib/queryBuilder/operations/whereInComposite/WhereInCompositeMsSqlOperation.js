...
class WhereInCompositeMsSqlOperation extends ObjectionToKnexConvertingOperation {
  ...
  buildComposite(knex, knexBuilder, columns, values) {
    if (Array.isArray(values)) {
      this.buildCompositeValue(knex, knexBuilder, columns, values);
    } else {
      this.buildCompositeSubquery(knex, knexBuilder, columns, values);
    }
  }

  buildCompositeValue(knex, knexBuilder, columns, values) {
    const helperColumns = columns.map(column =>
      `:${column.split('.').slice(-1)[0]}`)
    const subQuery = knex.from(
      knex.raw(`(VALUES ${
        values.map(value => `(${value.map(_ => '?').join(',')})`).join(',')
      }) AS V(${
        helperColumns.map(_ => '??').join(',')
      })`, flatten(values).concat(helperColumns)))
    .where(zipObject(helperColumns,
      columns.map(column => knex.raw('??', column))))

    if (this.prefix === 'not') {
      return knexBuilder.whereNotExists(subQuery);
    } else {
      return knexBuilder.whereExists(subQuery);
    }
  }
