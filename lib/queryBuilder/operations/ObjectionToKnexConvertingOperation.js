...
function convertArgs(op, builder, args) {
  ...
  for (let i = 0, l = args.length; i < l; ++i) {
    ...
    if (isUndefined(arg)) {
      ...
      out[i] = convertToKnexRaw(arg, knex);
...
function convertToKnexRaw(builder, knex) {
  return builder.toKnexRaw(knex);
}
...
function convertArray(arr, knex, i, op, skipUndefined) {
  ...
  for (let j = 0, l = arr.length; j < l; ++j) {
    ...
    if (item === undefined) {
      ...
      out.push(convertToKnexRaw(item, knex));
...
function convertPlainObject(obj, knex, i, op, skipUndefined) {
  ...
  for (let j = 0, l = keys.length; j < l; ++j) {
    ...
    if (item === undefined) {
      ...
      out[key] = convertToKnexRaw(item, knex);
    } else if (isObjectionQueryBuilderBase(item)) {
