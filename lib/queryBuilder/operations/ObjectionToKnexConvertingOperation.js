...
function convertArgs(op, builder, args) {
  ...
  for (let i = 0, l = args.length; i < l; ++i) {
    ...
    } else if (isObjectionLazyBuilder(arg)) {
      out[i] = convertLazyBuilder(arg, knex);
...
function convertUndefined(i, op, skipUndefined) {
  if (skipUndefined) {
    ...
    throw new Error(
      `undefined passed as argument #${i} for '${op.name}' operation. Call skipUndefined() method to ignore the undefined values.`
    );
  }
}

function isObjectionLazyBuilder(item) {
  return item !== null && item.isObjectionLazyBuilder === true;
}

function convertLazyBuilder(builder, knex) {
...
function convertArray(arr, knex, i, op, skipUndefined) {
  ...
  for (let j = 0, l = arr.length; j < l; ++j) {
    ...
    if (item === undefined) {
      if (!skipUndefined) {
        throw new Error(
          `undefined passed as an item in argument #${i} for '${op.name}' operation. Call skipUndefined() method to ignore the undefined values.`
        );
      }
    } else if (isObjectionLazyBuilder(item)) {
      out.push(convertLazyBuilder(item, knex));
...
function convertPlainObject(obj, knex, i, op, skipUndefined) {
  ...
  for (let j = 0, l = keys.length; j < l; ++j) {
    ...
    if (item === undefined) {
      if (!skipUndefined) {
        throw new Error(
          `undefined passed as a property in argument #${i} for '${op.name}' operation. Call skipUndefined() method to ignore the undefined values.`
        );
      }
    } else if (isObjectionLazyBuilder(item)) {
      out[key] = convertLazyBuilder(item, knex);
    } else if (isObjectionQueryBuilderBase(item)) {
