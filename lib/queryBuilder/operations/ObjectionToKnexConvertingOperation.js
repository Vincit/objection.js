...
function hasToKnexRawMethod(item) {
  return item !== null && typeof item.toKnexRaw === 'function';
}

function convertToKnexRaw(item, builder) {
  return item.toKnexRaw(builder.knex());
}
...
function convertArray(arr, builder, i, op, skipUndefined) {
  const out = [];
...
function convertPlainObject(obj, builder, i, op, skipUndefined) {
  const out = {};
