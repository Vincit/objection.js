...
class UpsertGraphOperation extends InsertGraphOperation {
  ...
  relate(builder) {
    ...
    // relateNodes.forEach((node) => console.log('relateNodes', node.upsertModel));
  ...
  update(builder) {
    ...
    // updateNodes.forEach((node) => console.log('updateNodes', node.upsertModel));
  ...
  upsertRecursively(builder) {
    ...
    // upsertRecursivelyNodes.forEach((node) => console.log('upsertRecursivelyNodes', node.upsertModel));
    ...
    return Promise.map(
      ...
      node => {
        return node.upsertModel.constructor
          .query()
          .upsertGraph(node.upsertModel, node.opt)
          .childQueryOf(builder, true);
      },
      { concurrency: this.graph.rootModelClass.concurrency }
    );
