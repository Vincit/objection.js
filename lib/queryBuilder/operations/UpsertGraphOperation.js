...
class UpsertGraphOperation extends InsertGraphOperation {
  ...
  onBefore1(builder) {
    this.graph = new UpsertGraph(this.models, this.queryProps, this.upsertOpt);
  ...
  onGraphInserterCreated(builder, graphInserter) {
    ...
    // as handled so that they are not inserted again.
    for (let i = 0, l = insertNodes.length; i < l; ++i) {
      ...
      if (upsertNode.type !== UpsertNode.Type.Insert && !insertNode.handled) {
        ...
        if (insertNode.parentNode) {
          ...
          parent.manyToManyConnections = parent.manyToManyConnections.filter(conn => conn.node !== insertNode);
        }

        insertNode.markAsHandled();
      }
    }
  }
  ...
  delete(builder) {
    return Promise.map(this.graph.nodes, node => {
      ...
      return Promise.map(relNames, relName => {
        ...
        const nodes = node.relations[relName].filter(it => it.type === UpsertNode.Type.Delete || it.type === UpsertNode.Type.Unrelate);
        const ids = nodes.map(it => it.currentModel.$id());

        if (ids.length) {
          const unrelate = nodes[0].type === UpsertNode.Type.Unrelate;
