...
class UpsertGraphOperation extends InsertGraphOperation {
  ...
  delete(builder) {
    return Promise.map(this.graph.nodes, node => {
      ...
      return Promise.map(relNames, relName => {
        ...
        const nodes = node.relations[relName].filter(it => it.hasType(UpsertNode.Type.Delete, UpsertNode.Type.Unrelate));
  ...
  update(builder) {
    ...
    const updateNodes = this.graph.nodes.filter(it => it.hasType(UpsertNode.Type.Update));
