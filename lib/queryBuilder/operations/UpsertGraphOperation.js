...
class UpsertGraphOperation extends InsertGraphOperation {
  ...
  onGraphInserterCreated(builder, graphInserter) {
    ...
    for (let i = 0, l = insertNodes.length; i < l; ++i) {
      ...
      if (upsertNode.type !== UpsertNode.Type.Insert) {
  ...
  onAfter2(builder, result) {
    return this.update(builder).then(() => super.onAfter2(builder, result));
  }
      ...
      node => {
        ...
        return Promise.map(
          ...
          relName => {
            ...
            const nodes = node.relations[relName].filter(
              it =>
                it.type === UpsertNode.Type.Delete ||
                it.type === UpsertNode.Type.Unrelate
            );
  ...
  update(builder) {
    ...
    const updateNodes = this.graph.nodes.filter(
      it => it.type === UpsertNode.Type.Update
    );
