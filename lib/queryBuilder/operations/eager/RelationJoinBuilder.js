...
class RelationJoinBuilder {
  ...
  build(builder) {
    ...
    this.doBuild({
      ...
      relation: null,
      path: '',
      selectFilterQuery: builderClone
    });
  }
  ...
  doBuild(args) {
    ...
    const selectFilterQuery = args.selectFilterQuery;
  ...
  buildSelects(args) {
    ...
    let columns = selectFilterQuery.findAllSelections().map(it => it.name);
    const selectAllIndex = columns.indexOf('*');

    // If there are no explicit selects, or there is a select all item,
    // we need to take select all columns using the schema information
    // in `RelationJoinBuilder.columnInfo`.
    if (columns.length === 0 || selectAllIndex !== -1) {
      columns.splice(selectAllIndex, 1);
      columns = RelationJoinBuilder.columnInfo[table].columns.concat(columns);
    }
      ...
      for (let i = 0, l = relation.joinTableExtras.length; i < l; ++i) {
        ...
        if (filterPassed) {
          ...
          if (!builder.hasSelection(fullCol, true)) {
            ...
            if (alias.length > ID_LENGTH_LIMIT) {
              throw modelClass.modelClass.createValidationError({
                type: ValidationErrorType.RelationExpression,
                message: `identifier ${alias} is over ${ID_LENGTH_LIMIT} characters long and would be truncated by the database engine.`
              });
            }
