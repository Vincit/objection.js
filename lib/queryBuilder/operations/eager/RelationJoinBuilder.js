...
class RelationJoinBuilder {
    ...
    return Promise.all(
      allModelClasses.map(ModelClass => {
        ...
        if (columnInfo[table]) {
          ...
          columnInfo[table] = columnInfoQuery.then(info => {
            const result = {
  ...
  doBuild(args) {
    ...
    if (!this.allRelations) {
      this.allRelations = findAllRelations(
        this.expression,
        this.rootModelClass
      );
    }
    ...
    if (!this.opt.minimize) {
      ...
      if (!encPath) {
        ...
        if (!path) {
          ...
          encPath = parts
            .map(part => this.opt.aliases[part] || part)
            .join(this.sep);
        }
  ...
  createIdGetter(modelClass, path) {
    const idCols = modelClass
      .getIdColumnArray()
      .map(col => this.joinPath(path, col));
...
function forEachExpr(expr, modelClass, callback) {
  ...
  if (
    expr.isAllRecursive() ||
    expr.maxRecursionDepth() > RELATION_RECURSION_LIMIT
  ) {
    throw modelClass.createValidationError({
...
function createThreeIdGetter(idCols) {
  ...
  return row => {
    ...
    if (
      isNullOrUndefined(val1) ||
      isNullOrUndefined(val2) ||
      isNullOrUndefined(val3)
    ) {
      return null;
    } else {
