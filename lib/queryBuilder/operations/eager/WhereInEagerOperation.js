...
class WhereInEagerOperation extends EagerOperation {
  ...
  onAdd(builder, args) {
    ...
    this.expression.forEachChildExpression(
      relations,
      (childExpression, relation) => {
        if (!relation) {
          throw modelClass.createValidationError({
            eager: `unknown relation "${childExpression.name}" in an eager expression`
          });
        }
    ...
    for (let i = 0, l = this.relationsToFetch.length; i < l; ++i) {
      ...
      for (let c = 0, lc = ownerProp.size; c < lc; ++c) {
        ...
        if (
          ...
          addedSelects.indexOf(fullCol) === -1
        ) {
  ...
  onAfter2(builder, result) {
    ...
    for (let i = 0, l = this.relationsToFetch.length; i < l; ++i) {
      ...
      const relationFetchPromise = this.fetchRelation(
        builder,
        models,
        relation,
        childExpression
  ...
  fetchRelation(builder, models, relation, expr) {
    ...
    return Promise.map(modelBatches, batch =>
      this.fetchRelationBatch(builder, batch, relation, expr)
    ).then(flatten);
  }

  fetchRelationBatch(builder, models, relation, expr) {
    const queryBuilder = this.createRelationQuery(
      builder,
      models,
      relation,
      expr
