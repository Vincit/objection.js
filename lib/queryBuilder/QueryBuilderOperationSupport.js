...
class QueryBuilderOperationSupport {
  constructor(knex) {
    const QueryBuilderContext = this.constructor.QueryBuilderContext;
    const userContext = QueryBuilderContext.newUserContext(this);

    this._knex = knex;
    this._operations = [];
    this._context = new QueryBuilderContext(userContext);
  }
  ...
  context(ctx) {
    ...
    if (arguments.length === 0) {
      ...
      const ctxBase = QueryBuilderContext.newUserContext(this);
  ...
  buildInto(knexBuilder) {
    ...
    for (let i = 0, l = this._operations.length; i < l; ++i) {
      ...
      if (this._operations.length !== l) {
        throw new Error(
          'onBuildKnex should only call query building methods on the knex builder'
        );
      }
    }
...
function forEachOperationInstanceOf(
  operations,
  operationSelector,
  callback,
  match
