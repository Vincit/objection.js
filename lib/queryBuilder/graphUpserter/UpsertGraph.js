...
class UpsertGraph {
  ...
  fetchCurrentState(builder) {
    // All "actual" model classes. Join table model classes etc are not included here.
    const allModelClasses = getAllModelClasses(this.rootModelClass, this.upsert);
    ...
    return builder  
      ...
      .context({
        onBuild(qb) { 
          const modelClass = qb.modelClass();
          const idCols = modelClass.getIdColumnArray();

          // TODO: Temporary hack to make sqlite work.
          const hackityHack = !isSqlite(qb) || !qb.hasSelection('_rowid_', true);

          // allModelClasses.has(modelClass) is there to skip all join table model classes
          // and other unrelated queries.
          if (allModelClasses.has(modelClass) && idCols.some(col => !qb.hasSelection(col, true)) && hackityHack) {
            qb.select(qb.fullIdColumnFor(modelClass));
          }
        }
      });
  }
  ...
  buildGraphStep(modelClass, upsert, current, parentNode, relExpr) {
    if (Array.isArray(upsert) || Array.isArray(current)) {
      this.buildGraphArray(modelClass, upsert, current, parentNode, relExpr);
    } else {
...
function getAllModelClasses(rootModelClass, graph) {
  const modelClasses = new Set();

  rootModelClass.traverse(graph, model => {
    modelClasses.add(model.constructor);
  });
