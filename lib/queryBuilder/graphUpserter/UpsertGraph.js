...
class UpsertGraph {
  ...
  build(builder) {
    return this.fetchCurrentState(builder).then(currentState =>
      this.buildGraph(currentState)
    );
  }
  ...
  fetchCurrentState(builder) {
    ...
    if (allowedExpr && !allowedExpr.isSubExpression(this.relExpr)) {
      throw builder.modelClass().createValidationError({
        allowedRelations: 'trying to upsert an unallowed relation'
      });
    }
      ...
      builder
        ...
        .then(result => {
          if (result.length !== rootIds.length) {
            throw new Error(
              `one or more of the root models (ids=[${rootIds.join(
                ', '
              )}]) were not found`
            );
          } else {
  ...
  buildGraphSingle(modelClass, upsert, current, parentNode, relExpr) {
    ...
    if (parentNode) {
      parentNode.relations[relExpr.name] =
        parentNode.relations[relExpr.name] || [];
    ...
    if (
      node.type === UpsertNode.Type.Delete ||
      node.type === UpsertNode.Type.Unrelate
    ) {
      return;
    }

    relExpr.forEachChildExpression(
      modelClass.getRelations(),
      (expr, relation) => {
...
function getRootIds(graph) {
  ...
  return graph.filter(it => it.$hasId()).map(root => root.$id());
}
