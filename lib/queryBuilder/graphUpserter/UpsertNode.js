...
class UpsertNode {
  ...
  constructor(parentNode, relExpr, upsertModel, currentModel, opt) {
    ...
    if (this.type === UpsertNodeType.Relate && !opt.relate) {
      throw new Error(`model (id=${this.upsertModel.$id()}) is not a child of model (id=${this.parentNode.upsertModel.$id()}). If you want to relate it, use the relate: true option`);
    }
