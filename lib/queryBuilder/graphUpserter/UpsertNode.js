...
class UpsertNode {
  constructor({ parentNode, relExpr, upsertModel, currentModel, dataPath, opt }) {
    ...
    // if (parentNode && parentNode.hasType(UpsertNodeType.Relate)) {
    //   throw new Error(
    //     `graph node (id=${parentNode.someModel.$id()}) is going to be related. cannot upsert any children of that node. objection doesn't support that yet. See the issue #782`
    //   );
    // }
  }
...
function getTypesRelate(node) {
  ...
  if (difference(props, rel.relatedProp.props).length !== 0) {
    ...
    if (relateType === UpsertNodeType.None) {
      ...
      return {
        types: [
          decideType(node, UpsertNodeType.Relate, 'noRelate'),
          decideType(node, UpsertNodeType.UpsertRecursively, 'noRelate')
        ]
      };
    } else {
      return {
...
function hasOption(node, optName) {
  ...
  // console.log('opt', opt);
  // console.log('node.relPathFromRoot', node.relPathFromRoot);
...
function getRelationPathFromRoot(node) {
  const path = [];
  // console.log('node.relExpr', node.relExpr);
...
function hasRelationsInUpsertModel(upsertModel) {
  ...
  for (let i = 0, l = relationArray.length; i < l; ++i) {
    ...
    if (upsertRelated) {
      changingProps.push(upsertRelated);
    }
  }

  return !!changingProps.length;
}
