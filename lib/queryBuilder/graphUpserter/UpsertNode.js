...
class UpsertNode {
  ...
  constructor(parentNode, relExpr, upsertModel, currentModel, queryProps, opt) {
    ...
    if (this.hasType(UpsertNodeType.Relate)) {
      const relation = this.relation;
      this.upsertModel[this.modelClass.dbRefProp] = this.upsertModel.$values(relation.relatedProp.props);
    }
  }
...
function getTypesRelate(node) {
  ...
  if (difference(props, rel.relatedProp.props).length !== 0) {
    ...
    const updateType = decideType(node, UpsertNodeType.Update, 'noUpdate');
...
function getTypesInsert(node) {
  return [
    decideType(node, UpsertNodeType.Insert, 'noInsert')
  ];
}
...
function getTypesUpdate(node) {
  ...
  if (changeType == ChangeType.NoChanges) {
    ...
    return [decideType(node, UpsertNodeType.Update, 'noUpdate')];
...
function decideType(node, defaultType, option, optionType) {
  return hasOption(node, option)
    ? optionType || UpsertNodeType.None
    : defaultType;
}
