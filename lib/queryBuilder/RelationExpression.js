...
class RelationExpression {
  ...
  isSubExpression(expr) {
    ...
    if (maxRecursionDepth > 0) {
      return (
        this.isAllRecursive() || this.maxRecursionDepth() >= maxRecursionDepth
      );
    }
    ...
    for (let i = 0, l = childAliases.length; i < l; ++i) {
      ...
      if (
        !ownSubExpression ||
        !ownSubExpression.isSubExpression(subExpression)
      ) {
        return false;
      }
    }
  ...
  childExpression(childAlias) {
    if (
      this.isAllRecursive() ||
      (childAlias === this.alias &&
        this._recursionDepth < this.maxRecursionDepth() - 1)
    ) {
      return new RelationExpression(
        this,
