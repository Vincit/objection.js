...
class QueryBuilder extends QueryBuilderBase {
  ...
  map(...args) {
    deprecate(
      'map is deprecated. Use the modify method instead. applyModifier will be removed in objection 3.0'
    );

    const promise = this.execute();
    return Bluebird.resolve(promise).map(...args);
  }

  reduce(...args) {
    deprecate('reduce is deprecated. reduce will be removed in objection 3.0');

    const promise = this.execute();
    return Bluebird.resolve(promise).reduce(...args);
  }

  catch(...args) {
    deprecate('catch is deprecated. catch will be removed in objection 3.0');

    const promise = this.execute();
    return Bluebird.resolve(promise).catch(...args);
  }

  return(...args) {
    deprecate('return is deprecated. return will be removed in objection 3.0');

    const promise = this.execute();
    return Bluebird.resolve(promise).return(...args);
  }

  reflect() {
    deprecate('reflect is deprecated. reflect will be removed in objection 3.0');
  ...
  bind(...args) {
    deprecate('bind is deprecated. bind will be removed in objection 3.0');

    const promise = this.execute();
    return Bluebird.resolve(promise).bind(...args);
  }

  asCallback(...args) {
    deprecate('asCallback is deprecated. asCallback will be removed in objection 3.0');

    const promise = this.execute();
    return Bluebird.resolve(promise).asCallback(...args);
  }

  nodeify(...args) {
    deprecate(
      'bind is nodeify. Use the modify method instead. applyModifier will be removed in objection 3.0'
    );

    const promise = this.execute();
    return Bluebird.resolve(promise).nodeify(...args);
  }
    ...
    if (err instanceof DuplicateRelationError) {
      throw modelClass.createValidationError({
        ...
        message: `Duplicate relation name "${
          err.relationName
        }" in relation expression "${exp}". For example, use "${
          err.relationName
        }.[foo, bar]" instead of "[${err.relationName}.foo, ${err.relationName}.bar]".`
      });
    } else {
      throw modelClass.createValidationError({
        type: ValidationErrorType.RelationExpression,
...
async function beforeExecute(builder) {
