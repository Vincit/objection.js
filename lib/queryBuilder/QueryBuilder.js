...
class QueryBuilder extends QueryBuilderBase {
  ...
  tableNameFor(modelClass, tableName) {
    ...
    if (typeof tableName === 'string') {
      ...
      return (
        (tableMap && tableMap[modelClass.tableName]) || modelClass.tableName
      );
    }
  }
  ...
  isExecutable() {
    ...
    return (
      !this._explicitRejectValue && !this._explicitResolveValue && !hasExecutor
    );
  }
  ...
  eager(exp, filters) {
    ...
    if (typeof this._eagerExpression === 'string') {
      this._eagerExpression = parseRelationExpression(
        this._modelClass,
  ...
  joinEager(exp, filters) {
    return this.eagerAlgorithm(this._modelClass.JoinEagerAlgorithm).eager(
      exp,
      filters
    );
  }
  ...
  naiveEager(exp, filters) {
    return this.eagerAlgorithm(this._modelClass.NaiveEagerAlgorithm).eager(
      exp,
      filters
    );
  }
  ...
  mergeJoinEager(exp, filters) {
    return this.eagerAlgorithm(this._modelClass.JoinEagerAlgorithm).mergeEager(
      exp,
      filters
    );
  }
  ...
  mergeNaiveEager(exp, filters) {
    return this.eagerAlgorithm(this._modelClass.NaiveEagerAlgorithm).mergeEager(
      exp,
      filters
    );
  }
  ...
  eagerOptions(opt) {
    this._eagerOperationOptions = Object.assign(
      {},
      this._eagerOperationOptions,
      opt
    );
  ...
  isFindQuery() {
    return (
      !this._operations.some(op => op.isWriteOperation) &&
  ...
  build(knexBuilder) {
    ...
    knexBuilder = buildInto(
      builder,
      knexBuilder || builder.knex().queryBuilder()
    );
  ...
  execute() {
    ...
    return promise.then(function() {
      ...
      if (builder._explicitRejectValue) {
        ...
        promise = Promise.resolve(builder._explicitResolveValue).bind(
          promiseCtx
        );
      } else if (queryExecutorOperation) {
        promise = queryExecutorOperation
  ...
  innerJoinRelation() {
    return this.addOperation(
      new JoinRelationOperation('innerJoinRelation', {
  ...
  outerJoinRelation(relationName) {
    return this.addOperation(
      new JoinRelationOperation('outerJoinRelation', {
  ...
  leftJoinRelation(relationName) {
    return this.addOperation(
      new JoinRelationOperation('leftJoinRelation', {
  ...
  leftOuterJoinRelation(relationName) {
    return this.addOperation(
      new JoinRelationOperation('leftOuterJoinRelation', {
  ...
  rightJoinRelation(relationName) {
    return this.addOperation(
      new JoinRelationOperation('rightJoinRelation', {
  ...
  rightOuterJoinRelation(relationName) {
    return this.addOperation(
      new JoinRelationOperation('rightOuterJoinRelation', {
  ...
  fullOuterJoinRelation(relationName) {
    return this.addOperation(
      new JoinRelationOperation('fullOuterJoinRelation', {
...
function callFindOperation(builder) {
  if (!builder.has(FindOperation)) {
    ...
    operation.opt = Object.assign(operation.opt, builder._findOperationOptions);
...
function buildInto(builder, knexBuilder) {
  ...
  const fromOperation = builder.findLastOperation(
    QueryBuilderBase.FromSelector
  );
...
const insertOperationFactory = createOperationFactory(
  InsertOperation,
  'insert'
);
const updateOperationFactory = createOperationFactory(
  UpdateOperation,
  'update'
);
const patchOperationFactory = createOperationFactory(UpdateOperation, 'patch', {
  modelOptions: {patch: true}
});
const relateOperationFactory = createOperationFactory(
  QueryBuilderOperation,
  'relate'
);
const unrelateOperationFactory = createOperationFactory(
  QueryBuilderOperation,
  'unrelate'
);
const deleteOperationFactory = createOperationFactory(
  DeleteOperation,
  'delete'
);
