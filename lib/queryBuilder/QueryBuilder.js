...
class QueryBuilder extends QueryBuilderBase {
  ...
  subqueryOf(query) {
    ...
    ctx.aliasMap = Object.assign({}, query.internalContext().aliasMap, aliasMap);
    ctx.tableMap = Object.assign({}, query.internalContext().tableMap, tableMap);
...
function parseRelationExpression(modelClass, expr) {
  try {
    ...
    throw modelClass.createValidationError({
      type: modelClass.ValidationError.Type.GenericInputValidation,
      data: {
        relationExpression: `Invalid relation expression "${expr}"`
      }
    });
  }
}
...
function checkEager(builder) {
  ...
  if (expr && allowedExpr && !allowedExpr.isSubExpression(expr)) {
    ...
    const message = 'eager expression not allowed';
